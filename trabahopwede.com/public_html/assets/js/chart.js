  ...
  fetch('../charts/chart-data.php')
    ...
    .then(data => {
      ...
      if (document.getElementById("applicantPieChart")) {
        ...
        window.applicantPieChart = new Chart(pieChartCanvas, {
          type: "pie",
          data: {
...
function generateDisabilityReport() {
  const labels = window.applicantPieChart?.data?.labels ?? [];
  const counts = window.applicantPieChart?.data?.datasets[0]?.data ?? [];
  const total = counts.reduce((a, b) => a + b, 0);
  let maxIndex = counts.indexOf(Math.max(...counts));
  let summary = "";
  counts.forEach((count, i) => {
    let percent = ((count / total) * 100).toFixed(2);
    summary += `${labels[i]}: ${percent}%\n`;
  });
  summary += `\nMost common: ${labels[maxIndex]}`;
  document.getElementById("disabilityReportText").textContent = summary;
  document.getElementById("disabilityReportModal").style.display = "block";
}

function generateVolunteerReport() {
  const labels = window.volunteerBarChart?.data?.labels ?? [];
  const counts = window.volunteerBarChart?.data?.datasets[0]?.data ?? [];
  let maxIndex = counts.indexOf(Math.max(...counts));
  let summary = `The month with the most volunteers is ${labels[maxIndex]} with ${counts[maxIndex]} volunteers.`;
  document.getElementById("volunteerReportText").textContent = summary;
  document.getElementById("volunteerReportModal").style.display = "block";
}


function filterByYear() {
  const year = document.getElementById("volunteerYear").value;
  fetch(`../charts/chart-data.php?year=${year}`)
    .then(res => res.json())
    .then(data => {
      if (window.volunteerBarChart) {
        window.volunteerBarChart.data.labels = data.volunteerLabels;
        window.volunteerBarChart.data.datasets[0].data = data.volunteerCounts;
        window.volunteerBarChart.update();
      }
    
      // PIE CHART: Applicants by Disability Type
      if (document.getElementById("applicantPieChart")) {
        const pieCtx = document.getElementById("applicantPieChart").getContext("2d");
        ...
        // Generate summary for pie chart
        const totalPie = data.pie.values.reduce((sum, v) => sum + v, 0);
        const topIndex = data.pie.values.indexOf(Math.max(...data.pie.values));
        const topLabel = data.pie.labels[topIndex];
        const pieSummary = `Total Applicants: ${totalPie}. Most common disability: ${topLabel}.`;
        const pieSummaryDiv = document.getElementById("summary-applicantPieChart");
        if (pieSummaryDiv) pieSummaryDiv.textContent = pieSummary;
      }

      // BAR CHART: Applicants by Month
      if (document.getElementById("applicantBarChart")) {
        const barCtx = document.getElementById("applicantBarChart").getContext("2d");
        ...
        // Generate summary for bar chart
        const totalBar = data.bar.values.reduce((sum, v) => sum + v, 0);
        const peakIndex = data.bar.values.indexOf(Math.max(...data.bar.values));
        const peakMonth = data.bar.labels[peakIndex];
        const barSummary = `Total Applicants: ${totalBar}. Peak in: ${peakMonth}.`;
        const barSummaryDiv = document.getElementById("summary-applicantBarChart");
        if (barSummaryDiv) barSummaryDiv.textContent = barSummary;
      }
});
}
